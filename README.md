...
To be re-rendered (only if they `extends React.PureComponent` or they implement a custom `componentShouldUpdate`).
...
I've only run with problem with this when using custom native components in React Native, but not on the web.
