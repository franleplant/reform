...
/***/ function(module, exports, __webpack_require__) {
	...
	function Reform(WrappedComponent) {
	  return class Reform extends _react2.default.Component {
	    ...
	    constructor(props) {
	      ...
	      this.errors = {};
	      this.state = {};
	      // Change handlers should get validation constrains from the event.target[props]
	    }

	    //genericHandleChange = (event) => {
	    //const name = event.target.name;
	    //if (!name) throw `Error: inputs need to have a 'name' attribute`

	    //if (!this.changeHandlers[name])
	    //this.changeHandlers[name] = function something() {}

	    //this.changeHandlers[name](event)
	    //}
	    ...
	    handlerFactory(field_name) {
	      return function handleChange(event) {
	        // - get the constraints
	        // - update the state
	        // - update the error object (probably need to put that into state)
	        let obj = {};
	    ...
	    render() {
	      ...
	      function reform(field_name) {
	        // - ensure props: handlers, errors, state values, etc are initializced
	        // - return props

	        if (!this.changeHandlers[field_name]) this.changeHandlers[field_name] = handlerFactory(field_name);

	        if (!this.errors[field_name]) this.errors[field_name] = '';

	        if (!this.state[field_name]) {
	          let obj = {};
	          obj[field_name] = handlerFactory(field_name);
	          this.setState(obj);
	        }

	        return {
	          onChange: this.changeHandlers[field_name],
	          value: this.state[field_name],
	          error: this.errors[field_name]
	        };
	      }

	      function error(field_name, callback) {
	        // - execute the callback like this: callback.call(null, errorHash[field_name])
	        // - return errorHash[field_name]
	      }

	      reform.error = error;

	      const props = Object.assign({}, this.props, { reform });
