...
test('validateField', () => {
  ...
  expect(core.validateField("", {})).toEqual({});
  expect(core.validateField("", {required: true})).toEqual({required: true});
  expect(core.validateField("", {required: true, email: true})).toEqual({required: true, email: false});
  expect(core.validateField("test", {required: true, email: true})).toEqual({required: false, email: true});
  expect(core.validateField("a@b.com", {required: true, email: true})).toEqual({required: false, email: false});

  expect(core.validateField("custom", {required: true, customValidator: value => value !== "custom"}))
    .toEqual({required: false, customValidator: false});

  expect(core.validateField("wrong", {required: true, customValidator: value => value !== "custom"}))
    .toEqual({required: false, customValidator: true});

  expect(core.validateField("", {required: true, customValidator: value => value !== "custom"}))
...
test('fieldIsValid(value, rules)', () => {
  ...
  expect(core.fieldIsValid('a@b.com', {email: true, required: true})).toBe(true)
